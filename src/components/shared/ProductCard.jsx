    ...
    <div className={`group ${className}`}>
      <div className="flex h-full flex-col overflow-hidden border border-deep-black bg-stark-white">
        ...
        <div className="px-4 pb-4">
          {p.inStock ? (
            <div className="flex w-full flex-col gap-1.5 sm:flex-row sm:gap-0">
              <button
                type="button"
                onClick={() => toggleWishlist(p.slug)}
                aria-pressed={saved}
                style={ANTON}
                className="w-full rounded-none border border-black bg-white py-3 text-[13px] uppercase tracking-widest text-black transition-none hover:bg-gray-100 sm:w-1/2"
              >
              ...
              <button
                ...
                onClick={() => addItem(p)}
                style={ANTON}
                className="w-full rounded-none border border-black bg-black py-3 text-[13px] uppercase tracking-widest text-white transition-none hover:bg-primary-red sm:w-1/2"
              >
                ADD TO CART →
              </button>
            </div>
